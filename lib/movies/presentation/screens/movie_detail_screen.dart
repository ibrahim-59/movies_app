  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<MovieDetailsBloc>()
        ...
        ..add(GetMovieRecommindationEvent(id)),
    ...
    return BlocBuilder<MovieDetailsBloc, MovieDetailsState>(
      builder: (context, state) {
        switch (state.moveDetailsState) {
          ...
          case RequestState.Loaded:
            return SliverGrid(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  final recommendation = state.movieRecommindations[index];
                  return FadeInUp(
                    from: 20,
                    duration: const Duration(milliseconds: 500),
                    child: ClipRRect(
                ...
                childCount: state.movieRecommindations.length,
              ),
          ...
          case RequestState.error:
            return Center(
              child: Text(state.movieRecommindationsMessage),
            );
        }
      },
    );
